# Final Merged Script: Enhanced Automotive Dashboard with Faker Data
...
kpi_fig = go.Figure()
kpi_fig.add_trace(go.Scatter(x=kpi_trend['Month'], y=kpi_trend['Sale Price'], name='Sale Price', line=dict(color='#4CAF50')))
kpi_fig.add_trace(go.Scatter(x=kpi_trend['Month'], y=kpi_trend['Commission Earned'], name='Commission', line=dict(color='#2196F3')))
...
trend_fig = px.line(trend_df, x='Quarter', y=['Sale Price', 'Commission Earned'], markers=True, template='plotly_dark', color_discrete_sequence=['#4CAF50', '#2196F3'])
...
with st.expander("🎞️ View Monthly Animated Trend", expanded=True):
    ...
    animated_fig = px.bar(melted, x='Metric', y='Amount', animation_frame='Month', template='plotly_dark', color='Metric', color_discrete_sequence=['#4CAF50', '#2196F3'])
...
with tab1:
    ...
    hr_data = pd.DataFrame({
        "Employee ID": [f"E{1000+i}" for i in range(20)],  # Increased to 20 records
    ...
    st.plotly_chart(px.histogram(hr_data, x="Performance Score", nbins=10, template="plotly_dark", color_discrete_sequence=['#4CAF50']), use_container_width=True)

with tab2:
    st.subheader("📦 Inventory Status")
    inventory_data = pd.DataFrame({
        "Part ID": [f"P{i:04d}" for i in range(1, 21)],  # Increased to 20 records
    ...
    st.bar_chart(low_stock.set_index("Part Name")["Stock Level"], color="#4CAF50")

with tab3:
    st.subheader("📞 CRM Interactions")
    crm_data = pd.DataFrame({
        "Customer ID": [f"C{100+i}" for i in range(20)],  # Increased to 20 records
    ...
    line_fig = px.line(line_chart_data, x="Contact Date", y="Satisfaction Score", markers=True, template="plotly_dark", color_discrete_sequence=['#4CAF50'])
    st.plotly_chart(line_fig, use_container_width=True)
    st.markdown("#### 😊 Satisfaction Score by Interaction Type")
    st.plotly_chart(px.box(crm_data, x="Interaction Type", y="Satisfaction Score", template="plotly_dark", color_discrete_sequence=['#4CAF50']), use_container_width=True)
...
with tab4:
    ...
    demo_data = pd.DataFrame({
        ...
        "Region": [fake.state() for _ in range(20)],  # Using US states for regions
    ...
    age_dist = px.histogram(demo_data, x="Age Group", color="Region", template="plotly_dark", color_discrete_sequence=px.colors.sequential.Greys)
    st.plotly_chart(age_dist, use_container_width=True)
    st.markdown("#### 💰 Purchase Amount by Region")
    region_purchase = px.box(demo_data, x="Region", y="Purchase Amount", template="plotly_dark", color_discrete_sequence=['#4CAF50'])
